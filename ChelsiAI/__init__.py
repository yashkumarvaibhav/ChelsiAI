...
try:
    ...
    from services.brain.decision_maker_api import make_decision_nlp, make_decision_ai, make_decision_chelsiai, \
        make_decision_chelsiai_string_matching
    ...
    from ChelsiAI.services.authenticate import verify_user
    from ChelsiAI.services.speech_to_text.speech_to_text import speech_to_text_google
    from ChelsiAI.services.text_to_speech.text_to_speech import text_to_speech
    from ChelsiAI.services.brain.decision_maker_api import make_decision_nlp, make_decision_ai, make_decision_chelsiai, \
        make_decision_chelsiai_string_matching
    from ChelsiAI.services.api_management_service.dist.manager import management as management_service
    from ChelsiAI.CONSTANT import Constant
    from ChelsiAI.features_default import dict_of_features, what_can_i_do
...
class ChelsiAI(InputsMethods, OutputMethods):
    def __init__(self, input_method: object, output_method: object, backend_tts_api='pyttsx3', api_key: str = "",
                 detect_wake_word: bool = True, wake_word_detection_method: object = None, bot_name: str = "Chelsi",
    ...
    def register_feature(self, feature_obj: object, feature_command: str):
        """
        Register a feature to ChelsiAI
    ...
    def validate_input_output_option(self):
        ...
        if self.input_method not in [InputsMethods.text_input, InputsMethods.voice_input_google_api,
            ...
            try:
                ...
                from ChelsiAI.ChelsiAI.services.asr_deepspeech_streaming.streaming import listen as deepspeech_listen, \
                    ARGS, \
                    vad_audio, model
        ...
        if self.api_key is None or self.api_key == "":
            raise ValueError(
                "Invalid api_key. Expected a valid api_key.")
        else:
            if not os.path.exists(CONSTANT.USER_CONFIG_FOLDER):
                os.mkdir(CONSTANT.USER_CONFIG_FOLDER)
            ...
            if self.wake_word_detection_method is None:
                ...
                if self.deepspeech_listen_obj is None:
                    ...
                    try:
                        ...
                        from ChelsiAI.ChelsiAI.services.asr_deepspeech_streaming.streaming import \
    ...
    def manage_tasks(self):
        ...
        if inp in list(self.custom_features.keys()):
            ...
            task, accuracy_ = make_decision_chelsiai(self.api_key, inp)
        ...
        if accuracy_ > 0.5:
            ...
            task = make_decision_chelsiai_string_matching(classes, inp)
            fun = dict_of_features[task]
            print("===========>", task.upper(), "<===========")
...
if __name__ == "__main__":
    ...
    obj = ChelsiAI(input_method=InputsMethods.text_input,
